...
export const getStories = () => async (dispatch) => {
	...
	try {
		const response = await Axios.get(
			`http://reachnbuy.unitechitsolution.in/AndroidClass/get_stores_list.php`,
		);
		dispatch({
...
export const getOffers = () => async (dispatch) => {
	...
	try {
		const response = await Axios.get(
			`http://reachnbuy.unitechitsolution.in/AndroidClass/get_offer.php`,
		);
		dispatch({
...
export const getNews = () => async (dispatch) => {
	...
	try {
		const response = await Axios.get(
			`http://reachnbuy.unitechitsolution.in/AndroidClass/reachnbuynews.php`,
		);
		dispatch({
			type: GET_NEWS_FETCH,
